...
// these rects and `CompositionCoach` judges inside them, so the frame the coach
// approves is the frame that ships.
import CoreGraphics

enum PublishCrop {
    /// The upright capture frame's aspect (w/h) — the `.photo` preset's 3:4.
    static let captureAspect: CGFloat = 3.0 / 4.0

    /// Reels / TikTok / Shorts / the Tovis Looks feed.
    static let feed: CGFloat = 9.0 / 16.0
    /// Instagram's tallest feed post.
    static let instagramFeed: CGFloat = 4.0 / 5.0

    /// The centered crop of `aspect` (w/h) inside a capture frame of
    /// `frameAspect`, as a normalized TOP-LEFT rect of that frame.
    static func rect(aspect: CGFloat, frameAspect: CGFloat = captureAspect) -> CGRect {
    ...
    static var feedRect: CGRect { rect(aspect: feed) }
    ...
    static let coverSafeTopFraction: CGFloat = 220.0 / 1920.0
    static let coverSafeBottomFraction: CGFloat = 450.0 / 1920.0
    ...
    /// layer's points).
    static func coverSafeRect(in feedRect: CGRect) -> CGRect {
    ...
    static func inCropSpace(_ r: CGRect, crop: CGRect) -> CGRect {
    ...
    static func inCropSpace(_ p: CGPoint, crop: CGRect) -> CGPoint {
