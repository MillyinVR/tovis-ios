...
- [ ] Board-creation context parity (web PR tovis-app#511, personalization spec §7–8): board type chips + event date + skippable chip questions in the board create/save-to-board flow, ~~and the "N days until …" countdown on a board~~. API is additive (`type`/`eventDate`/`answers` on POST/PATCH `/api/v1/boards*`; question sets in `tovis-app/lib/boards/context.ts` are the SSOT) — iOS keeps working untouched, boards it creates default to GENERAL, so parity is deferred, not blocking. **The countdown half is DONE — iOS #148** (parity-gaps step 7): `BoardEventCountdownSection` on `BoardDetailView`, with the ported rules on the TovisKit model (`BoardCatalog.eventNoun`/`wantsEventDate`, `BoardEventDate`, `BoardEventCountdownState`) + an inline date editor (`PATCH …/boards/{id}`). `CreateBoardView` already ships the chips/date/questions, so what remains here is the **save-to-board** flow (`SaveToBoardSheet`). **The save-to-board flow now opens `CreateBoardView` natively (parity-gaps step 18)** — the empty state no longer points to tovis.app, and creating a board there auto-saves the look into it (web "Create and save"). Remaining under this item: the self-profile write-through (next item).
...
- [x] ~~Board-feed "Recommended for this board" parity~~ **DONE — iOS #148** (2026-07-16, parity-gaps epic step 7). `BoardRecommendationsSection` in `BoardDetailView` fetches the owner-only `GET /api/v1/boards/{id}/feed` via `BoardsService.recommendations(id:limit:)` and renders it with the existing `LooksFeedItem` model — the deferral reason ("iOS has no board detail screen yet") expired once `BoardDetailView` shipped. Paging (`cursor`/`seen`) is deliberately not wired: the card shows one fixed set, matching web.
...
- [ ] **Audit the ~11 remaining hand-rolled `yyyy-MM-dd` formatters for the UTC off-by-one** (found in iOS #148, parity-gaps step 7). A `YYYY-MM-DD` wire field is a **calendar date**, but `DatePicker` hands back an *instant* in the **local** calendar carrying the current time of day — so formatting it with a `TimeZone(identifier: "UTC")` formatter sends the **next day** for any pick after 5pm in PDT. That was a live bug in `CreateBoardView` (a board counted down to the wrong date); #148 fixed the **board** field only, via `BoardEventDate.ymd/date(fromYmd:)/daysUntil` in TovisKit (calendar-injectable, defaults to `.current`, DST-proof, regression-pinned both ways). Each remaining site needs the same question — *is this date user-**picked**, and is it converted in UTC?*: `BookingFlowView` (×2), `ProVerificationView`, `ClientProfileEditView`, `ProSignupView`, `ProExpenseFormView`, `ProOpenSlotPicker`, `ProClientTechnicalEditSheets`, `NotificationsView`, `ProNotificationsView`, `TovisKit/ProCalendar/ProCalendarGrid`. **The smell is `TimeZone(identifier: "UTC")` next to a `DatePicker`.** (A formatter only ever *reading back* a server-sent ymd is fine.) Natural fix: promote the board helper to a general TovisKit calendar-date helper and migrate the picked-date sites onto it.
...
- [ ] **Fixture contract validation has drifted and nothing gates it** (found round-3 step 19).
  ...
  `tovis-app/schema/api/tovis-api.schema.json`) only ever runs when someone remembers to.
  **6 fixtures currently fail**, several from round-3's own web PRs: `clientHome.json`
  (`invites[].opening.publicIncentive`, #678), `clientMe.json` (`user.availableWorkspaces`,
  #669), `publicClaim.json` (`professionalName`), `proProfile.json` (`header.coverUrl`) and
  `looksFeed.json` ×2 (`focalX`/`focalY`). None is a runtime bug — the app decodes these
  optionally — but the harness that exists to catch wire drift is currently blind. Fix is two
  parts: refresh the fixtures, and decide whether to add a CI workflow (or a pre-push hook) so
  `npm run validate` actually gates. ⚠️ It needs the sibling `tovis-app` checkout for the schema,
  which is why it was never wired.
- [ ] ~~Route the `.tovis.me` pill to the native profile.~~ **DISSOLVED — Tori's call, round-3
  step 19.** `ProProfileTabView:219`'s "Open ↗" **stays on system Safari**. The native route
  already exists as "View as client ›" at `:158` (60 lines above, same screen), so routing the
  pill would duplicate an adjacent affordance *and* remove the only way a pro can confirm their
  paid vanity URL actually resolves — `{handle}.tovis.me` rewrites to `/p/[handle]`, which renders
  the same `PublicProfileView` as `/professionals/[id]`. The pill is premium-only. Recorded so the
  next audit doesn't re-chase it.
